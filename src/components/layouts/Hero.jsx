...
const Hero = ({
  ...
  return hero2 ? (
    ...
    <section
      ref={sectionRef}
      className={`px-[30px] relative flex flex-row ${
        imgRight ? "mb-[180px] pb-[30px]" : "mb-[150px]"
      } pt-24 w-full`}
    >
      <div
        className={`bg-cover bg-center bg-no-repeat flex flex-nowrap w-full shrink rounded-[20px] relative ${
          imgRight ? "px-10" : "px-0"
        }`}
        style={imgLeft ? {} : { backgroundImage: `url(${banners_home_bg})` }}
      >
        <div
          className={`flex grow shrink justify-end flex-row ${
        ...
        >
          ...
          {!imgLeft && imgRight && (
            <div className="w-full md:w-[46%] flex flex-col grow-0 shrink relative">
              <motion.div style={{ y }}>
                <div
                  className="-mb-[200px] bg-cover bg-center bg-no-repeat relative shadow-[0_0_20px_10px_rgba(0,0,0,0.1)] flex min-h-[800px] rounded-[600px_600px_20px_20px] flex-col grow-0 shrink"
